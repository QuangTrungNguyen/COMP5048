...
def addEdge(origin, count, year, month, date):
   return '<edge source="' + origin + '" target="EWR" start="' + year + '-' + month + '-01" end="' + year + '-' + month + '-' + date + '" weight="' + count + '" />'
...
for year in range(2000,2004):
   for month in range(1,13):
      output += '\n'
      c.execute('SELECT Origin, Count(*) From flight Where Dest = "EWR" and Year = ' + str(year) + ' and Month = ' + str(month) + ' Group by Origin')
      arr = c.fetchall()
      c.execute('SELECT Dest, Count(*) From flight Where Origin = "EWR" and Year = ' + str(year) + ' and Month = ' + str(month) + ' Group by Dest')
      dep = c.fetchall()
...
with open('EWR_edges.TXT', 'w') as edge:
   edge.write(output)
