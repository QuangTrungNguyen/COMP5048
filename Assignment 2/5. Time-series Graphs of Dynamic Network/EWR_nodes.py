...
airports = []
c.execute('SELECT Distinct Origin from flight Where Dest = "EWR" and Year != 1999 Order by Origin')
origin = c.fetchall()
c.execute('SELECT Distinct Dest from flight Where Origin = "EWR" and Year != 1999 Order by Dest')
dest = c.fetchall()
...
for i in range(0,len(airports)):
   ...
   c.execute('SELECT count(*), Month, Year from flight Where (Origin = "EWR" and Dest = "' + airports[i] + '" and Year != 1999) or (Origin = "' + airports[i] + '" and Dest = "EWR" and Year != 1999) Group by Year,Month')
   data = c.fetchall()
...
with open('EWR_nodes.TXT', 'w') as ewr:
   ewr.write(final_output)