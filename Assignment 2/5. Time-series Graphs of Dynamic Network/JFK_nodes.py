...
airports = []
c.execute('SELECT Distinct Origin from flight Where Dest = "JFK" and Year != 1999 Order by Origin')
origin = c.fetchall()
c.execute('SELECT Distinct Dest from flight Where Origin = "JFK" and Year != 1999 Order by Dest')
dest = c.fetchall()
...
for i in range(0,len(airports)):
   ...
   c.execute('SELECT count(*), Month, Year from flight Where (Origin = "JFK" and Dest = "' + airports[i] + '" and Year != 1999) or (Origin = "' + airports[i] + '" and Dest = "JFK" and Year != 1999) Group by Year,Month')
   data = c.fetchall()
...
with open('JFK_nodes.TXT', 'w') as jfk:
   jfk.write(final_output)
